...
Class importDataPlaceObjects
    ...
    Sub Main()
            ...
            If result IsNot Nothing Then
                ...
                Select Case selection
                    Case "Ausleger", "Spurhalter", "Spurhalterabzug"
                        ImportData(n_max)
                        GenerateModel(selection, n_max)
                    Case "Importiere nur Daten"
                        ImportData(n_max)
                        GenerateBKS(n_max)
                    Case "Generiere alle Modelle"
                        ImportData(n_max)
                        GenerateModel("Ausleger", n_max)
                        GenerateModel("Spurhalter", n_max)
                        GenerateModel("Spurhalterabzug", n_max)
                    Case "Importiere Daten und generiere alle Modell"
                        ImportData(n_max)
                        GenerateBKS(n_max)
                        GenerateModel("Ausleger", n_max)
                        GenerateModel("Spurhalter", n_max)
        ...
        Catch ex As Exception
            Logger.Error("Fehler in Main: " & ex.Message & vbCrLf & ex.StackTrace)
        End Try
    End Sub

    Function SelectFDKType() As Tuple(Of String, Integer)
        Dim validFilterValues As String() = {"Bitte eine Auswahl treffen", "Importiere nur Daten", "Importiere Daten und generiere alle Modell", "Generiere alle Modelle", "Ausleger", "Spurhalter", "Spurhalterabzug"}
    ...
    Sub GenerateBKS(n_max As Integer)
        Logger.Info("GenerateBKS gestartet.")
        ...
        For i = n_min - 1 To n_max - 1
            ...
            If ((xe(i) > xd(i) And yn(i) > yd(i)) Or yd(i) < yn(i)) Then 'Check direction of rotation
            ...
            Dim oOcc As ComponentOccurrence = oAsmCompDef.Occurrences.Add(ThisDoc.Path & "..\..\SKLT\sklt.iam", oMatrix)
    ...
    Sub GenerateModel(selection As String, n_max As Integer)
        Logger.Info("Generate gestartet für: " & selection)
        ...
        For i = n_min - 1 To n_max - 1
            ...
            ElseIf selection = "Spurhalter" Then
            ...
            ElseIf selection = "Spurhalterabzug" Then
        ...
        '[ Set instance values
        For i = n_min - 1 To n_max - 1
            If typ(i) = "Typ10A" Or typ(i) = "Typ10R" Or typ(i) = "Typ21A" Or typ(i) = "Typ21R" Then '& " xx" > makes sure iProperties is a text
                If selection = "Ausleger" Then
    ...
    Sub EndOfRoutine()
        ...
        Catch ex As Exception
            Logger.Error("Fehler in EndOfRoutine: " & ex.Message & vbCrLf & ex.StackTrace)
        End Try
    End Sub
End Class
